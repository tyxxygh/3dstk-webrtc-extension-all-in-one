...
CHDIR = 'winuwp-app-type-revision'
...
if (sys.platform == 'win32' and
  ...
  test.run_gyp('winuwp-app-type-revision.gyp', chdir=CHDIR)

  test.build('winuwp-app-type-revision.gyp', 'enable_winuwp_81_revision_dll',
             chdir=CHDIR)

  # Revision is set to 8.2 which is invalid for 2013 projects so compilation
  # must fail.
  test.build('winuwp-app-type-revision.gyp', 'enable_winuwp_82_revision_dll',
             chdir=CHDIR, status=1)

  # Revision is set to an invalid value for 2013 projects so compilation
  # must fail.
  test.build('winuwp-app-type-revision.gyp', 'enable_winuwp_invalid_revision_dll',
             chdir=CHDIR, status=1)
